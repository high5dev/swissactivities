...
const BookingWidget = ({
  ...
  useEffect(() => {
    const presetData = async () => {
      setLoading(true);
      const formattedData = await formatAvailabilities(availabilitiesList);

      if (firstAvailableDate || availableDates[0]) {
        const firstDate = new Date(firstAvailableDate || availableDates[0]);

        if (formattedData?.length >= 1) {
          setDate(firstDate);
          setActiveOffer(formattedData[0]);
          if (formattedData[0].availabilities) {
            setActiveAvailability(formattedData[0].availabilities[0]);
            handleTicketsUpdate({
              ticket: formattedData[0].availabilities[0].ticketCategories[0],
              amount: 1,
            });
          }
        } else {
          onSelectBookingDate(firstDate, true);
        }
      }
    ...
    presetData();
  }, [availabilitiesList, availableDates, firstAvailableDate]);
  ...
  const clearSelected = (newData=[], isInitial) => {
    ...
    if (!isInitial && prevSelectedTime) {
      ...
      const isTicketsListChanged = clearSelectedTickets(
        activeOfferTicketCategories
      );

      if (isTicketsListChanged) {
        goToStep(2);
      }
  ...
  const formatOfferItem = async (offer) => {
    ...
    try {
      ...
      if (contentOffer) {
        ...
        offer.offerLabel;
        const description =
        translation?.description || contentOffer.data.offer?.description || "";
  ...
  const onSelectBookingDate = async (e, isInitial) => {
    setLoading(true);
  ...
  const disableCalendarTile = ({ activeStartDate, date, view }) => {
    return (
      ...
      (date.setHours(0, 0, 0, 0) <
        firstAvailableDateObject.setHours(0, 0, 0, 0) ||
  ...
  return (
    <div className={styles.BookingWidget} onClick={handleWidgetActivation}>
      ...
      <div className={styles.checkoutHeader}>
        ...
        {isMobile && lastVisitedStep < 2 && !isBookingActive && (
          <Button
            title={t("activity.widget.check")}
            onClick={mobileCheckAvailability}
      ...
      >
        {(currentStep === 3 || (isMobile && isBasketReached)) && (
          ...
          <div className={styles.totalTeaser}><StaticImage src={activity.teaser_image?.url} alt="teaser" width="250" height="250" layout="responsive" className={styles.totalTeaserImage}/></div>
      ...
      <div className={styles.paymentMethodsBoxMobile}>
